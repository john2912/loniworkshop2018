...
int main(int argc, char ** argv)
{
    ...
    //TODO: complete the following collective call
    MPI_Scatter(     ,       ,MPI_REALNUM,sub_mat,       ,MPI_REALNUM,root,MPI_COMM_WORLD);
    ...
    MPI_Type_hvector(nrows,1,ext_float,one_col,&mul_cols);
    MPI_Type_commit(&mul_cols);
    //TODO: change the extent of mul_cols to resized type rs_mul_cols
    MPI_Type_create_resized(mul_cols,0,               ,&rs_mul_cols);
    ...
    if (rank==0) {
        ...
        recvp = &(AT[0][0]);
        for (i=0;i<psize;i++) {
            //TODO: complete calculation of recv_counts and recv_displs
            recv_counts[i]= ;
            recv_displs[i]= ;
        }
    }

    //TODO: complete the collective call using gatherv
    MPI_Gatherv(sub_mat,       ,MPI_REALNUM,recvp,recv_counts,recv_displs,rs_mul_cols,root,MPI_COMM_WORLD);
