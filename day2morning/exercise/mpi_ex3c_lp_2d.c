...
int main(int argc, char** argv)
{
    ...
    //TODO: complete the top and bottom user defined types
    MPI_Datatype bc_top_bottom;
    MPI_Type_contiguous(         ,MPI_REALNUM,&bc_top_bottom);
    MPI_Type_commit(&bc_top_bottom);

    //TODO: complete the left and right user defined types
    MPI_Datatype bc_left_right;
    MPI_Type_vector(         ,1,           ,MPI_REALNUM,&bc_left_right);
    ...
    for (iter=1;iter<=niter && (dt_global > relerr) ;iter++) {
        ...
        //TODO: complete the Send/Recv calls for boundary
        // receive the outer boundaries, i.e. halo cells
        MPI_Irecv(&(told[ ][           ]),1,bc_left_right,rk_right, 0,MPI_COMM_WORLD, &req_r);
        MPI_Irecv(&(told[ ][ ]),          1,bc_left_right,rk_left,  0,MPI_COMM_WORLD, &req_l);
        MPI_Irecv(&(told[ ][ ]),          1,bc_top_bottom,rk_top,   0,MPI_COMM_WORLD, &req_t);
        MPI_Irecv(&(told[           ][ ]),1,bc_top_bottom,rk_bottom,0,MPI_COMM_WORLD, &req_b);

        // send the inner boundary values
        MPI_Send(&(told[ ][         ]), 1, bc_left_right, rk_right, 0, MPI_COMM_WORLD);
        MPI_Send(&(told[ ][ ]),         1, bc_left_right, rk_left,  0, MPI_COMM_WORLD);
        MPI_Send(&(told[ ][ ]),         1, bc_top_bottom, rk_top,   0, MPI_COMM_WORLD);
        MPI_Send(&(told[         ][ ]), 1, bc_top_bottom, rk_bottom,0, MPI_COMM_WORLD);
        ...
        MPI_Send(&dt,1,MPI_REALNUM,0,0,MPI_COMM_WORLD);
        // exchange info among blocks
        if (rank == 0) {
            MPI_Recv(&dt_global,1,MPI_REALNUM,0,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE); 
            for (i=1;i<psize;i++) {
                MPI_Recv(&dt,1,MPI_REALNUM,i,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
                if (dt_global < dt)
                    dt_global = dt;
            }

            // like a MPI_bcast for dt_global to all process
            for (i=1;i<psize;i++)
                MPI_Send(&dt_global,1,MPI_REALNUM,i,0,MPI_COMM_WORLD);
