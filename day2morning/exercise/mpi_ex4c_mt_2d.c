...
int main(int argc, char ** argv)
{
    ...
    if (rank==0) {
        ...
        for (i=0;i<psize;i++) {
            ...
            // TODO: complete the calculation of send_counts[] and send_displs[]
            send_counts[i]= ;
            grid_row_id =           ;
            grid_col_id =           ;
            send_displs[i] =                                 ;
    ...
    //TODO: complete the scatter call
    MPI_Scatterv(sendp,send_counts,send_displs,rs_smat_orig,sub_mat,   ,MPI_REALNUM,root,MPI_COMM_WORLD);
    ...
    //TODO: complete the user defined type
    MPI_Type_vector( , ,  ,MPI_REALNUM,&one_col);
    MPI_Type_create_hvector( , ,        ,one_col,&smat_trans);
    MPI_Type_create_resized(smat_trans,0,          ,&rs_smat_trans);
    ...
    if (rank==root) {
        ...
        for (i=0;i<psize;i++) {
            ...
            // TODO: calculate the col_id and row_id of the process grid
            grid_col_id =           ;
            grid_row_id =           ;
            recv_displs[i]=                                ; 
    ...
    //TODO: complete the collective function call
    MPI_Gatherv(sub_mat,   ,MPI_REALNUM,recvp,recv_counts,recv_displs,rs_smat_trans,root,MPI_COMM_WORLD);
    free(sub_mat);
