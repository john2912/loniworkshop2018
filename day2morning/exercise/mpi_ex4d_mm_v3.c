...
int main (int argc, char **argv)
{
 ...
 //   real flops,init_time,start_time,end_time;
    ...
    if (argc==6) {
        ...
        //ns_rows=3, nca=5, ns_cols=7;
        //ndiv_cols=3, ndiv_rows=2;
        //tot_rows=ns_rows*ndiv_rows, tot_cols=ns_cols*ndiv_cols;
...
//    start_time = MPI_Wtime();
    ...
    if (myrank ==0) {
        // TODO: complete the Scatterv parameters:
        int row_id;
        for (i=0;i<nprocs;i++){
            send_counts[i]= ;
            row_id =                ;
    ...
    //TODO: complete the resized datatype
    MPI_Type_create_resized(smat_b,0,                ,&rs_smat_b);
    ...
    //TODO: complete the gatherv parameters
    if (myrank==0) {
        for (i=0;i<nprocs;i++)
            recv_counts[i]=1;
        int row_id,col_id;
