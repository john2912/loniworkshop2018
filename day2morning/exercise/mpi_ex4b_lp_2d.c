...
int main(int argc, char** argv)
{
    ...
    // A send to MPI_PROC_NULL succeeds and returns as soon as possible.
    // A receive from MPI_PROC_NULL succeeds and returns as soon as possible with no modifications to the receive buffer
    //left bc
    if (j_blk == 0) {
    ...
    for (iter=1;iter<=niter && (dt_global > relerr) ;iter++) {
        ...
        //TODO comment out some lines below, and finish this collective function
        MPI_     (     ,     , ,MPI_REALNUM,      ,MPI_COMM_WORLD);
